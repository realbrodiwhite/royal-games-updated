...
let keepThrowingCoins: boolean = true;
async function throwCoins(stage: PIXI.Container): Promise<void> {
const coins: PIXI.AnimatedSprite[] = [];
  for (let i = 0; i < 50 && keepThrowingCoins; i++) {
const coin: PIXI.AnimatedSprite = PIXI.AnimatedSprite.fromFrames(PIXI.Assets.cache.get('coin-animation-spritesheet').data.animations.coin);
    coin.x = 1280 / 2;
    coin.y = 788;
    ...
    stage.addChild(coin);
    coin.play();
let moveXStep: number = 50 + Math.random() * 80;
const coinMovementTimeline: gsap.core.Timeline = gsap.timeline();
...
function checkForFreeSpins(winningLine: number[]): void {
  const mostValuableSymbol = symbolsCount; // Assuming the most valuable symbol is the last one
  const count = winningLine.filter(symbol => symbol === mostValuableSymbol).length;
  if (freeSpinsBonus[count]) {
    game.triggerFreeSpins(freeSpinsBonus[count]);
  }
}

const prizeWheel = new PrizeWheel(game.stage, ['100 Coins', '200 Coins', '300 Coins', '400 Coins', '500 Coins']);
...
const btnSpinWheel = game.addButton([
  'spin_button1.png',
  'spin_button2.png',
  'spin_button3.png',
  'spin_button4.png',
